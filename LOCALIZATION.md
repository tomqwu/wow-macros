...
每个职业的所有宏都保存在一个 `macros.md` 文档中。如果宏包含本地化的法术、物品、天赋、光环或其他游戏文本，应在同一宏条目下使用上述标题放置英文和简体中文两个代码块。两个版本的条件与功能应完全一致，只有本地化名称可以不同。仅当所有内容在两个客户端中都无需翻译时，才使用一个客户端通用代码块。
