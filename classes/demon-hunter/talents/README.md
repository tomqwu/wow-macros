# Demon Hunter talents

Specializations: Devourer, Havoc, Vengeance.

Store one talent loadout per Markdown file. Use the specialization and build purpose in a lowercase kebab-case filename.

Example: `<specialization>-<build>.md`

Record the game version or patch, verification date, intended content, import string, and any usage notes in each build.
