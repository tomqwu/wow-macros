...
| 字段 | 内容 |
| --- | --- |
| 游戏版本 | WoW TBC Classic，第二阶段团队副本环境；具体客户端版本未提供 |
| 职业 | 术士 |
| 等级 | 未提供 |
| 天赋配置 | 主要为暗毁，偶尔使用火毁；具体点数分配未提供 |
| 关键天赋 | 来源包含暗影灼烧、燃烧、恶魔牺牲，以及其他替代配置或功能技能 |
| 职责 / 玩法 | 团队 DPS、诅咒分配、仇恨、AoE、焦点控制、宠物管理、召唤、消耗品和功能支持 |
| 循环来源 | 导入的 `zhCN` 宏包包含暗毁和火毁团队优先级、AoE、仇恨规则与按键设置 |
| 客户端语言 | 简体中文（`zhCN`） |
| 最后更新 | 2026-08-19 |
| 总体状态 | 59 个 `zhCN` 宏代码块仍为 `imported-reference`；尚无双语宏组合 |

## 技能循环与天赋模型

### 玩家循环

- 起手：来源提供暗毁和火毁的诅咒与伤害准备，但尚未选定当前配置。
- 持续优先级：来源记录暗影箭或烧尽路线、火毁的献祭与燃烧、诅咒分配、AoE 和仇恨规则。
- 冷却 / 应急：来源包含饰品、毁灭药水、灵魂碎裂、恢复消耗品、宠物牺牲、控制和召唤。

### 宏设计机会

- 可在确定暗毁或火毁、分配诅咒、当前宠物、战斗职责和目标偏好后生成精简宏组合。
- 从这些选择中提炼宠物控制、焦点控制、诅咒、DPS、仇恨、消耗品和召唤宏。

### 明确保留手动操作

- 伤害优先级、持续伤害刷新、诅咒分配变化、仇恨决定和宠物选择保持手动。
- 在未确定当前天赋与宠物计划前，不把暗毁、火毁和各宠物专用操作合并成一套玩家配置。

## 宏组合

尚无双语宏。需要先选定毁灭天赋路线与宠物计划，再添加并测试匹配的 `enUS` 版本。

## 导入参考资料

> 状态：`imported-reference`

- 来源范围：另一会话提供的 TBC 术士宏包
- 来源语言：简体中文（`zhCN`）
- 游戏版本：WoW TBC Classic，第二阶段团队副本环境；具体客户端版本未提供
- 导入日期：2026-08-18
- 内容：59 个 Lua 宏代码块，以及法术对照、按键、循环、仇恨和 TBC 兼容性说明
- 验证状态：保留来源会话中的说法；尚未独立复核宏行为、本地化宠物条件、等级和物品名称
- 语言完整性：目前仅有 `zhCN` 来源；尚未生成配套 `enUS` 发布版本
- 隐私：原始资料中的角色名行已按要求省略

发布前必须在目标 TBC 客户端中测试本地化法术、物品和宠物条件名称，拆分超过客户端字符限制的宏，并按照 [本地化规范](../../../LOCALIZATION.md) 添加匹配版本。以下来源正文保留了移除外层复制代码围栏和角色标识、调整标题层级以及清理行尾空格后的内容。
