...
| 字段 | 内容 |
| --- | --- |
| 游戏版本 | WoW TBC Classic / TBC Anniversary；未提供准确客户端版本 |
| 职业 | 圣骑士 |
| 等级 | 70 |
| 天赋配置 | 源资料涉及惩戒、防护和神圣；未提供单一天赋点法 |
| 关键天赋 | 源资料包含十字军打击、复仇者之盾、神圣震击、神恩术及其他天赋专属操作 |
| 职责 / 玩法 | 惩戒任务与团队副本、防护坦克、神圣治疗、通用辅助和工程物品 |
| 循环来源 | 导入的 `zhCN` 宏包包含惩戒圣印舞时机，以及防护和神圣优先级 |
| 客户端语言 | 简体中文（`zhCN`） |
| 最后更新 | 2026-08-19 |
| 总体状态 | 多天赋 `zhCN` 源资料仍为 `imported-reference`；尚无双语配对宏组合 |

## 技能循环与天赋模型

### 玩家循环

- 起手：资料包含使用审判和圣印准备的惩戒团队副本起手；尚未整理防护和神圣起手。
- 持续优先级：惩戒圣印舞、防护仇恨技能和神圣治疗选择分别记录在源资料中。
- 冷却 / 应急：资料包含复仇之怒、饰品、神恩术治疗、免疫技能、祝福和圣疗术。

### 宏设计机会

- 确定当前天赋点法和职责后，再生成一套对应天赋的宏。
- 根据选定循环和辅助计划整理圣印、审判、鼠标指向治疗、嘲讽、祝福、免疫和物品宏。

### 明确保留手动操作

- 圣印舞的武器摆动时机、审判决策、坦克仇恨优先级和治疗目标选择由玩家控制。
- 不把互斥天赋的宏混合提升为同一套玩家配置。

## 宏组合

目前没有双语配对宏。先选择一个实际圣骑士天赋，再仅为相关源宏补齐 `enUS` 版本。

## 导入参考资料

> 状态：`imported-reference`

- 来源范围：另一会话提供的 `paladin_macros.md` 整理稿
- 来源语言：简体中文（`zhCN`）
- 游戏版本：WoW TBC Classic / TBC Anniversary
- 职业与等级：70 级圣骑士
- 专精：惩戒、防护、神圣
- 导入日期：2026-08-18
- 验证状态：保留源会话中的说法；尚未独立复核宏行为、法术等级或本地化名称
- 语言完整度：仅有 `zhCN` 源资料；尚未生成配对的 `enUS` 正式版本

标记为已验证前，必须在目标 TBC 客户端版本中重新检查，核对对应客户端中的法术与物品名称，并按 [LOCALIZATION.md](../../../LOCALIZATION.md) 添加配对版本。若客户端等级格式不同，应从法术书中用 Shift 点击插入准确的中文法术等级。以下导入正文完整保留。
